...
## Benchmark Evidence (2025-11-01)
- Toolchain: Rust 1.90.0 (release build)
- Dataset: synthetic fixture with 100k small files generated by `scripts/benchmark.sh`
- Release binary size: **4.20 MB** (under 6 MB target)
- Optimized scan runtime: **851 ms**
- Legacy scan runtime: **635 ms** (optimized path remains within parity guardrail; workload metadata-bound)
- View runtime (top 100): **68 ms**
- View drill-down runtime (`--path dir1`): **65 ms**
- Snapshot size: 5.0 MiB
- Command log captured via `./scripts/benchmark.sh` and appended to `benchmark-results.txt`
