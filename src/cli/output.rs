...
/// Calculate size ratio for display purposes (intentionally allows precision loss for large sizes)
#[allow(clippy::cast_precision_loss)]
fn size_ratio(size: u64, total: u64) -> f64 {
    if total == 0 {
        0.0
    } else {
...
impl PreviewStrategy for AdaptivePreviewStrategy {
    fn should_preview(
        ...
        let parent_ratio = size_ratio(entry.size_bytes, parent_size);

        // Check depth-based threshold
        for (d, threshold) in &self.depth_thresholds {
            if depth < *d && parent_ratio >= *threshold {
                return true;
            }
        }

        // Fallback for deeper levels
        if depth >= self.depth_thresholds.last().map_or(3, |(d, _)| *d) && parent_ratio >= 0.60 {
            return true;
        }
...
pub fn format_text_with_all_entries(
    ...
    print_entries_recursive(entries, all_entries, strategy, root_size, root_size, 0);
...
pub fn format_text_with_strategy(
    summary: &Summary,
    entries: &[DirectoryEntry],
    strategy: &dyn PreviewStrategy,
) {
    ...
    // Print entries with hierarchical preview
    print_entries_recursive(entries, &[], strategy, root_size, root_size, 0);
...
fn print_entries_recursive(
    ...
    for (rank, entry) in current_entries.iter().enumerate() {
        ...
        let pct = size_ratio(entry.size_bytes, root_size) * 100.0;
